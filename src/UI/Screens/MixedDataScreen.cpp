...
MixedDataScreen::MixedDataScreen(Adafruit_ST7789* screen){
    _screen = screen;
    ...
    _outTempTile = new MetricTile(_screen,161,0,159,72,false,1);
    _outTempTile->setupGraph(true, true, TileGraphColor::PURPLE);
    _outTempTile->setTitle("out t");

    _outPMTile = new MetricTile(_screen,161,74,159,72,false);
...
void MixedDataScreen::showBackendWeatherData(PresentingBackendWeatherData backendWeatherData){
    std::vector<TileDataItem> tileData(DATA_COLLECTION_CAPACITY);
...
TileStatus MixedDataScreen::mapWarningLevelToTileStatus(WarningLevel level){
